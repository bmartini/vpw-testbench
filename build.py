...
def parse_header(name) -> Dict[str, str]:
    ...
    PORTS = (IN8 | IN16 | IN32 | IN64 | INW |
            OUT8 | OUT16 | OUT32 | OUT64 | OUTW).desc('variable width definition')

    NAME = regex('[a-zA-Z]+\w*').desc('variable name')
    ...
    for line in lines:
        try:
            port_def, _ = seq(PORTS, NAME).parse_partial(line)
